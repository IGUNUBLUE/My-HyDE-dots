...
| active | HyDE themes, fonts, theme switching | [LAGC Calm theme pair](decisions/2026-09-21-lagc-calm-themes.md) | Warm low-glare Dark/Light pair with static theme.dcol palettes (fixes wallpaper-derived color mismatch), Inter Medium UI font with Atkinson Next SemiBold on Waybar, per-app reload coverage after theme switches. |
| active | VSCodium, editor themes | [VSCodium LAGC themes](decisions/2026-09-22-vscodium-calm-themes.md) | VS Code-compatible lagc-themes extension (Calm + Tech pairs, Tech palette desaturated ~28% for eye comfort) packaged via codium --install-extension; Kiro and Zed support removed. |
| active | OpenCode, Codex, Wallbash | [Agent CLI wallbash themes](decisions/2026-09-25-agent-cli-wallbash-themes.md) | always/ .dcol + selector scripts regenerate and pin a `hyde-wallbash` theme in opencode cli.json and codex config.toml on every switch; kitty ANSI palette spliced for semantic colors; SIGUSR2 gated by SigCgt. |
| active | GTK3, Chromium, browsers, Wallbash | [GTK prefer-dark pin](decisions/2026-09-25-gtk-prefer-dark-mode-pin.md) | always/ hook writes `gtk-application-prefer-dark-theme` in gtk-3.0 settings.ini from `wallbash_mode` and re-emits gtk-theme so GTK-mode Chromium flips light/dark without restart. |
