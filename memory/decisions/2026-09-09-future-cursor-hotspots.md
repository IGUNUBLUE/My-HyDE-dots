...
- 2026-09-22 (later): several redesigns of the arrow were prototyped and all rejected after live review — sharpened/BMZ-style tips (look deformed at 32px), a from-scratch ship+flame arrow (user: "feo, incompleto"), and accent→flame/comet variants (read as a droplet or needle, not propulsion). **Reverted to the upstream rounded arrow**; only the hotspot corrections above remain. External themes tested live and rejected: Shipsmall, Layan (original + a `Layan-future` cyan recolor built with the full SVG→XCursor/Hyprcursor pipeline). Layan note: XCursor-only themes render invisible under Hyprland — a native `hyprcursors/` dir built via `hyprcursor-util` is required. Reference notes kept: XCursor image chunks are 9 uint32 fields `(size,type,nominal,version=1,w,h,xhot,yhot,delay=50)` + premultiplied BGRA pixels; PNGs render via `rsvg-convert` at 32/40/48/64 px under nominals 24/30/36/48; same-name `hyprctl setcursor` can serve stale textures — cycle to another theme and back to force a reload; running clients cache their own XCursor theme and need a restart.
