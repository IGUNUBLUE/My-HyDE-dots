...
`Atkinson Hyperlegible` (package `ttf-atkinson-hyperlegible`) backs `[waybar]`, `[desktop.ui]`, `[gtk3]`, `[qt5]`, `[qt6]`, and `[notification]` font keys in `config.toml`; HyDE applies fonts globally so this also affects LAGC Tech. `CaskaydiaCove Nerd Font Mono` remains monospace. `install.sh` fetches each Calm wallpaper from Wallhaven (dark `7jwx8v`, originally Flickr peste76; light `ly8xwr`, originally Unsplash `m1Jnyio2pUs`) and falls back to a deterministic ImageMagick gradient when offline; no wallpaper binary is committed. `wall.hyprlock.png` stays a manual symlink convention; HyDE reads `~/.cache/hyde/wallpapers/hyprlock.png`.
